...
<Tabs groupId='schema-builder'>
  <Tab label="valibot">
我们可以使用 [valibot](/zh/guide/schema-integration/valibot) 来定义 `hello` 操作的返回类型：
```ts
import { loom } from "@gqloom/core"
...
```
在上面的代码中，我们使用 `v.sting()` 来定义 `hello` 操作的返回类型，`valibotSilk` 函数让我们把 `valibot` 的 Schema 定义作为`丝线`使用。

我们还可以直接使用从 `@gqloom/valibot` 导出的 `resolver` 和 `query` 函数，以使得代码更加简洁：
...
我们还可以使用 [zod](/zh/guide/schema-integration/zod) 来定义 `hello` 操作的返回类型：
```ts
import { loom } from "@gqloom/core"
import { z } from "zod"
...
刚刚我们定义的 `BookResolver` 对象可以通过 [weave](/zh/guide/fundamentals-features/weave) 函数编织成 GraphQL schema：
