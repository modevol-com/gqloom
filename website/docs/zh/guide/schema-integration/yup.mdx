...
我们还可以使用元数据中的 `asField` 属性为字段添加元数据，例如 `description` 等：
...
在上面的代码中，我们为 `age` 字段添加了 `description` 元数据，最终得到如下 GraphQL Schema：
...
使用来自 `@gqloom/yup` 的 `union` 声明联合类型，例如：
```ts
...
我们可以使用 `string().oneof()` 来声明枚举类型，例如：
...
首先我们使用 `ValibotWeaver.config` 来定义类型映射的配置。这里我们导入来自 [graphql-scalars](https://the-guild.dev/graphql/scalars) 的 `GraphQLDateTime`，当遇到 `date` 类型时，我们将其映射到对应的 GraphQL 标量。
```ts
