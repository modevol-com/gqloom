...
```ts
import { resolver, query, field, weave } from '@gqloom/valibot'
...
在上面的代码中，我们使用 `valibotSilk` 来定义输入类型，
`v.object({ id: v.number() })` 定义了输入对象的类型，
`asInputArgs()` 将这个对象标注为 GraphQL 的输入参数，
...
`async (next) => { ... }` 定义了一个中间件，
`useLoggedUser()` 是一个自定义的函数，用于获取当前登录的用户，
