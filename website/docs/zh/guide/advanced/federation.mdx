...
从 `@gqloom/federation` 引入的 `FederatedSchemaWeaver` 函数用于编织 Federation Schema。与 `@gqloom/core` 相比，`@gqloom/apollo` 中的 `FederatedSchemaWeaver` 函数将输出携带指令（Directives）的 Schema。
