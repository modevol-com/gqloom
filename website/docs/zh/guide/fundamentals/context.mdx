...
在 Node.js 世界中，上下文（Context）允许我们在同一个请求中共享数据和状态。在 GraphQL 中，上下文允许在同一个请求的多个[解析函数](../fundamentals/resolver)和[中间件](../fundamentals/middleware)之间共享数据。
...
你可以在[解析器](../fundamentals/resolver)内的任何地方调用 `useContext` 函数以访问当前请求的上下文，而不需要显式地传递 `context` 函数。
...
要了解更多关于中间件的信息，请参阅 [中间件文档](../fundamentals/middleware)。
