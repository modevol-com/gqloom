...
<PackageManagerTabs command="install @gqloom/core @gqloom/mikro-orm @mikro-orm/core" />
...
In the code above, we first define a `Giraffe` entity and then create a bobbin using `MikroResolverFactory`. From the bobbin, we can generate GraphQL operations directly.
