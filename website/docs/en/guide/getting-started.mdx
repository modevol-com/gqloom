...
export const schema = weave(HelloResolver, CatResolver) // [!code hl]
```
...
<Tabs>
  <Tab label="valibot">
```ts title="src/index.ts"
import { resolver, query, mutation, field } from "@gqloom/valibot"
import * as v from "valibot"
...
- Check out the core concepts of GQLoom: [silk](./fundamentals-features/silk), [resolver](./fundamentals-features/resolver), [weave](./fundamentals-features/weave);
- Understanding common features: [context](./fundamentals-features/context), [DataLoader](./fundamentals-features/dataloader), [Middleware](./fundamentals-features/middleware)
