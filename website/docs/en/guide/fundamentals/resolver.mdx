...
const HelloResolver = resolver({
  hello: query(valibotSilk(v.string()), () => "Hello, World"),
})
```
In the code above, we use `v.sting()` to define the return type of the `hello` operation, and the `valibotSilk` function lets us use the Schema definition of `valibot` as a `silk`.

We can also use the `resolver` and `query` functions exported from `@gqloom/valibot` directly to make the code more clean:
...
We can also define the return type of the `hello` operation using [zod](. /schema-integration/zod) to define the return type of the `hello` operation:
```ts
import { loom } from "@gqloom/core"
import { z } from "zod"
...
const HelloResolver = resolver({
  hello: query(zodSilk(z.string()), () => "Hello, World"),
})
```
...
We can also use the `resolver` and `query` functions exported from `@gqloom/zod` directly to make the code more clean:
...
The `BookResolver` object we just defined can be woven into a GraphQL schema using the [weave](../fundamentals/weave) function:
```ts
import { weave } from '@gqloom/core'

export const schema = weave(BookResolver)
```
