...
<Tabs items={["Relational API v2", "Relational API v1"]}>
<Tab>
...
Adding computed properties to a database table is quite simple. However, it's important to use the `field().derivedFrom()` method to declare the columns on which the computed property depends, so that the `useSelectedColumns` method can correctly select these columns: 
