/*
 * Original code from [@pothos/plugin-directives](https://github.com/hayes/pothos/tree/main/packages/plugin-directives)
  ...
  schema.extensionASTNodes = [
    {
      ...
      directives: directiveNodes(
        schema.extensions?.directives as DirectiveList
      ),
      operationTypes: (
        [
          {
  ...
  Object.keys(types).forEach((typeName) => {
    ...
    if (type instanceof GraphQLObjectType) {
      type.astNode = {
        ...
        interfaces: type
          .getInterfaces()
          .map((iface) => typeNode(iface) as NamedTypeNode),
        fields: fieldNodes(type.getFields()),
        directives: directiveNodes(
          type.extensions?.directives as DirectiveList
        ),
      }
...
function typeNode(type: GraphQLType): TypeNode {
  ...
  if (type instanceof GraphQLNonNull) {
    return {
      ...
      type: typeNode(type.ofType as GraphQLType) as
        | ListTypeNode
        | NamedTypeNode,
    }
  }
